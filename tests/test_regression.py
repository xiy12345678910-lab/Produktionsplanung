...
s_, code, _ = put(SA, to_pm); check(s_ == 403, f"V12.7.2: keine Übergabe Vertrieb -> PM mehr (Eingang zählt als PM) ({s_} {code})")
def sales_offer(n):
...
s_, code, _ = put(P, pm_offer); check(s_ == 403, f"V12.7.2: 'Angebot versendet' ist kein Schritt mehr ({s_} {code})")
def sales_accept(n):
    p = plog(n, "pj1", "t_sales"); p["phase"] = "accepted"; p["ab"] = "AB-777"; p["dueDate"] = "2026-11-30"; audit_entry(n, "t_sales")
s_, code, _ = put(SA, sales_accept); check(s_ == 403, f"V12.7.2: Vertrieb übergibt NICHT an die Produktion ({s_} {code})")
def accept_nodate(n):
...
s_, code, _ = put(P, accept); check(s_ == 200, f"V12.7.2: PM übergibt an Produktion mit AB + Liefertermin ({s_} {code})")
def sales_after(n):
